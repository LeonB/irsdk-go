...
func BenchmarkSortInt128New(b *testing.B) {
	rand.Seed(1)
	x := make([][2]int64, nItem)
	for i := 0; i < b.N; i++ {
		for j := range x {
			x[j][0] = int64(rand.Intn(nItem * 10))
		}
		Sort(x, func(i, j int) bool {
			return x[i][0] < x[j][0]
		})
...
func BenchmarkSortInt128Old(b *testing.B) {
	rand.Seed(1)
	x := make([][2]int64, nItem)
	for i := 0; i < b.N; i++ {
		for j := range x {
...
// Int128Slice attaches the methods of sort.Interface to []int64, sorting in increasing order.
type Int128Slice [][2]int64

func (s Int128Slice) Len() int           { return len(s) }
func (s Int128Slice) Less(i, j int) bool { return s[i][0] < s[j][0] }
